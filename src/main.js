...
async function loadHistory() {
    ...
    el.historyContainer.innerHTML = '<p class="text-center text-gray-400 py-20 text-base">載入中...</p>';
    try {
        const q = query(
        ...
        el.historyContainer.innerHTML = `<p class="text-center text-red-400 py-20 text-sm font-bold">載入失敗: ${err.message}</p>`;
    }
}
...
function renderHistory() {
    ...
    if (filtered.length === 0) {
        el.historyContainer.innerHTML = '<p class="text-center text-gray-400 py-20 text-base">找不到相符的紀錄</p>';
        return;
    }
    ...
    filtered.forEach(d => {
        ...
        itemEl.className = "bg-white p-4 rounded-[1.5rem] shadow-sm flex gap-4 items-center border border-gray-50";
        itemEl.innerHTML = `
            <div class="w-20 h-20 bg-gray-100 rounded-2xl overflow-hidden flex-shrink-0 cursor-zoom-in border border-gray-50">
                ${d.imageUrl ? `<img src="${d.imageUrl}" class="w-full h-full object-cover img-trigger">` : `<div class="w-full h-full flex items-center justify-center text-gray-300"><svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg></div>`}
            </div>
            <div class="flex-grow">
                <h3 class="font-bold text-gray-800 text-base leading-tight mb-1">${d.itemName}</h3>
                <p class="text-xs text-gray-400 font-bold tracking-wide">${d.date}</p>
            </div>
            <div class="text-right flex flex-col items-end gap-2">
                <p class="text-blue-600 font-black text-xl tracking-tighter">$${d.price}</p>
                <button class="btn-edit text-xs font-bold bg-blue-50 text-blue-500 px-3 py-1.5 rounded-lg active:bg-blue-100" 
                        data-id="${d.id}" data-name="${d.itemName}" data-price="${d.price}">
                    修改
                </button>
            </div>
        `;
...
el.btnClearFilter.onclick = () => {
    el.searchKeyword.value = ""; el.filterStart.value = ""; el.filterEnd.value = "";
    renderHistory();
};
...
el.inputCamera.onchange = async (e) => {
    ...
    if (file) {
        ...
        el.previewImg.src = selectedBase64; el.previewImg.classList.remove('hidden');
        el.cameraPlaceholder.classList.add('hidden'); el.btnRemoveImg.classList.remove('hidden');
    }
};
el.btnRemoveImg.onclick = (e) => {
    e.stopPropagation();
    selectedBase64 = ""; el.inputCamera.value = "";
    el.previewImg.classList.add('hidden'); el.cameraPlaceholder.classList.remove('hidden');
...
el.addForm.onsubmit = async (e) => {
    ...
    try {
        ...
        alert("儲存成功！");
        el.addForm.reset(); el.inputDate.valueAsDate = new Date(); el.btnRemoveImg.click();
...
el.editForm.onsubmit = async (e) => {
    ...
    try {
        ...
        el.modalEdit.classList.add('hidden'); loadHistory();
...
onAuthStateChanged(auth, (user) => {
    currentUser = user;
    if (user) {
        el.userSection.innerHTML = `<img src="${user.photoURL}" class="w-10 h-10 rounded-full border shadow-sm"><button id="btn-logout" class="text-xs text-gray-400 font-bold underline">登出</button>`;
        document.querySelector('#btn-logout').onclick = () => signOut(auth);
        switchView('add');
    } else {
        el.userSection.innerHTML = ''; switchView('login');
    }
});
